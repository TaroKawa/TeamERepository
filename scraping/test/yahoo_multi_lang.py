...
def dir_manage(size, color, natinality):
    color_dic = {
        "Red": "赤",
        "Orange": "オレンジ",
        "Yellow": "黄色",
    ...
    file_path = f"../../data/{natinality}/{color_jp}/{size}"
    if not os.path.isdir(f"../../data/{natinality}/{color_jp}"):
        os.mkdir(f"../../data/{natinality}/{color_jp}")
...
def retreive_img(li, size, color, natinality):
    file_path = dir_manage(size, color, natinality)
    if not file_path:
        return
    n = len(li)
...
def img_query(query, nationality):
    driver, disp = access(query)
    ...
    for i in range(1, n_colors):
        for j in range(1, n_sizes):
            ...
            retreive_img(li, size, color, nationality)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--nationality", default="ja")
    ...
    img_query(q_vs_n_dic[args.nationality], args.nationality)
